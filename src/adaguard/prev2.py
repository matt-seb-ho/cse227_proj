...
# Reasonable caps to avoid hyper-long generations that might stall
REQUEST_TIMEOUT = 120  # seconds per request
MAX_TOKENS = 4096  # enough for long reasoning but not insane
MAX_RETRIES = 5
...
def call_with_retries(
    ...
    """
    Wrapper around client.chat.completions.create with retry + timeout.
    ...
    for attempt in range(max_retries):
        try:
            resp = client.chat.completions.create(
                ...
                timeout=REQUEST_TIMEOUT,  # <-- hard wall-clock timeout
            )
            return resp
...
def classify_prompt(
    ...
    try:
        ...
        # On total failure (including repeated timeouts), mark and skip
...
def main():
    ...
    with open(jsonl_path, "a", encoding="utf-8") as jsonl_f:
        for i, row in enumerate(tqdm(ds, desc="Classifying with gpt-oss-20b")):
            ...
            # Correctness (unknown is treated as incorrect)
